...
export default function OrderNotificationBell() {
    ...
    const loadOrders = async () => {
        try {
            const [pendingRes, completedKitchenRes] = await Promise.all([
            ...
            const completedKitchenData = Array.isArray(completedKitchenRes?.data?.data)
                ? completedKitchenRes.data.data
                : Array.isArray(completedKitchenRes?.data)
            ...
            const completedKitchenOrders = completedKitchenData
                .filter(order => {
                ...
                .map(order => ({
                    ...order,
                    notificationType: "COMPLETED_KITCHEN",
                }));

            setOrders([...pendingOrders, ...completedKitchenOrders]);
        } catch (error) {
            console.error(
    ...
    useEffect(() => {
        ...
        eventSource.addEventListener("connected", event => {
            try {
                console.log("SSE NOTIFICATION CONNECTED:", JSON.parse(event.data));
            } catch {
        ...
        eventSource.addEventListener("order.deleted", handleOrderEvent);
        eventSource.addEventListener("table.updated", handleOrderEvent);
                        ...
                        <div className="max-h-[500px] overflow-y-auto">
                            {orders.length === 0 ? (
                                ...
                                orders.map(order => {
                                    ...
                                    const isCompletedKitchen =
                                        order.notificationType === "COMPLETED_KITCHEN";
                                    ...
                                    return (
                                        <div
                                            key={order.id}
                                            className={
                                                "m-2 border-b p-4 last:border-b-0 " +
                                                (isCompletedKitchen
                                                    ? "bg-green-50"
                                                    : "bg-gray-100")
                                            }
