...
export default function FoodPanel({
    ...
    const loadData = async () => {
        try {
            const foodRes = mode === "customer"
                ? await foodService.getByQrCode(qrCode)
                : await foodService.getByBranch();
    ...
    return (
        <div className="flex h-full flex-col">
            ...
            {mode === "customer" && cart.length > 0 && (
                <button
                    onClick={onOpenCart}
                    className="fixed bottom-6 right-6 z-40 rounded-full bg-[#4f7d4f] px-6 py-4 font-semibold text-white shadow-xl transition hover:bg-[#416b41]"
                >
