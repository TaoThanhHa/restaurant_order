...
export default function InvoicePanel({ order, reload, onAddFood, table }) {
    ...
    const oldItems = orderItems.filter(item => item.status !== "PENDING");
    const newItems = orderItems.filter(item => item.status === "PENDING");
    ...
    const handleServed = async () => {
        try {
            setErrorMessage("");
            await orderService.closeOrder(order.id, "SERVED");
            await reload();
        } catch (err) {
            setErrorMessage(
                err.response?.data?.message ||
    ...
    const handleCancelOrder = async () => {
        try {
            setErrorMessage("");
            await orderService.closeOrder(order.id, "CANCELLED");
            setConfirmAction(null);
            await reload();
        } catch (err) {
            setConfirmAction(null);
    ...
    return (
        <div className="flex h-full flex-col pt-2">
            ...
            <div className="flex-1 overflow-y-auto p-5">
                {oldItems.length > 0 && (
                    <>
                        <h3 className="mb-3 font-semibold">Món đã gửi bếp</h3>
            ...
            <div className="border-t pl-5 pr-5">
                ...
                {order.status === "PENDING" && newItems.length > 0 && (
                    <Button
            ...
            {confirmAction && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                    <div className="w-[400px] rounded-xl bg-white p-5 shadow-xl">
                        <h3 className="text-lg font-bold">Xác nhận hủy</h3>
