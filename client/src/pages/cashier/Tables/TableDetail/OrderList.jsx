...
export default function OrderList({
    ...
    const getPendingItems = order =>
        (order.orderItems || []).filter(item => item.status === "PENDING").length;

    const hasServedItem = order =>
        (order.orderItems || []).some(item => item.status === "SERVED");
    ...
    const requestCancelOrder = (order, event) => {
        ...
        if (!canCancelOrder(order)) {
            setErrorMessage("Không thể hủy đơn vì đơn đã có món được phục vụ.");
            return;
        }
    ...
    const handleCancelOrder = async () => {
        ...
        try {
            setLoading(true);
            setErrorMessage("");
            await orderService.closeOrder(cancelOrder.id, "CANCELLED");
            setCancelOrder(null);
    ...
    return (
        <div className="flex h-full flex-col">
            <div className="m-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500">{totalOrders} đơn đang hoạt động</p>
                    </div>
                </div>

                <div className="flex gap-2">
                    {orders.length > 1 && (
                        <Button className="w-[100px]" onClick={onMergeOrders}>
                            Gộp đơn
                        </Button>
                    )}

                    <Button className="flex w-[90px] items-center justify-center" onClick={onCreateOrder}>
                        + Order
                    </Button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
                {orders.length === 0 && (
                    <div className="p-10 text-center text-gray-400">Chưa có đơn nào.</div>
                )}
                ...
                {orders.map(order => {
                    ...
                    const foodCount = (order.orderItems || []).filter(
                    ...
                    return (
                        <div
                            ...
                            className={
                                "cursor-pointer border-b p-4 transition hover:bg-gray-50 " +
                        ...
                        >
                            ...
                            {canCancel && (
                                <div className="mt-3 flex justify-end">
                                    <button
                                        type="button"
                                        onClick={event => requestCancelOrder(order, event)}
            ...
            {cancelOrder && (
                <div
                    ...
                    onClick={() => !loading && setCancelOrder(null)}
                >
                    <div
                        className="w-[400px] rounded-xl bg-white p-5 shadow-xl"
                        onClick={event => event.stopPropagation()}
                    >
                        <h3 className="text-lg font-bold">Xác nhận hủy đơn</h3>

                        <p className="mt-3 text-sm text-gray-600">
                            Bạn có chắc muốn hủy đơn{" "}
                            <span className="font-semibold text-gray-800">
                                {cancelOrder.orderCode || "#" + cancelOrder.id}
                            </span>
                            ?
                        </p>

                        <p className="mt-2 text-sm text-red-500">
                            Tất cả món trong đơn sẽ được hủy.
                        </p>
