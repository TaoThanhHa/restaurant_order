...
export default function CustomerManagement() {
    ...
    const scopeConfig = {
        multi: {
            label: "Toàn bộ nhà hàng",
            description:
                "Hiển thị khách hàng và dữ liệu từ tất cả chi nhánh",
        },
        single: {
            label: "Nhà hàng hiện tại",
            description:
                "Hiển thị khách hàng thuộc chi nhánh duy nhất của nhà hàng",
        },
        branch: {
            label: "Chi nhánh của bạn",
            description:
                "Hiển thị khách hàng thuộc chi nhánh đang quản lý",
        },
    };
    ...
    const handlePeriodChange = (newPeriod) => {
        ...
        if (newPeriod === "month") {
            setPeriodValue(String(new Date().getMonth() + 1));
            return;
        }

        if (newPeriod === "quarter") {
            setPeriodValue(String(getCurrentQuarter()));
            return;
        }
    ...
    const loadCustomers = async () => {
        try {
            ...
            const result = await customerAdminService.getAll(params);
    ...
    const handleSelectCustomer = async (customer) => {
        try {
            ...
            const result = await customerAdminService.getById(
                customer.id,
                params
            );
            ...
            <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div>
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="inline-flex items-center gap-1.5 rounded-full bg-[var(--color-secondary)] px-3 py-1 text-xs font-medium text-[var(--color-primary)]">
                            <Store size={14} />
                            {currentScope.label}
                        </span>
                    </div>
                ...
                <div className="flex flex-col gap-4 border-b border-slate-100 p-5 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-[var(--color-text-muted)]">
                                {customers.length}
                            </span>
                        </div>
                        ...
                        <p className="mt-1 text-xs text-[var(--color-text-muted)]">
                            {scopeType === "multi"
                                ? "Theo dõi khách hàng trên toàn bộ các chi nhánh"
                                : scopeType === "single"
                                ? "Theo dõi khách hàng của nhà hàng và chi nhánh duy nhất"
                                : "Theo dõi khách hàng của chi nhánh đang quản lý"}
                        </p>
                    </div>
...
function CustomerDetail({
    ...
    const branches = Array.isArray(customer.branches)
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <span className="rounded-full bg-[var(--color-secondary)] px-2.5 py-1 text-xs font-medium text-[var(--color-primary)]">
                                        {scopeText}
                                    </span>
                                </div>
