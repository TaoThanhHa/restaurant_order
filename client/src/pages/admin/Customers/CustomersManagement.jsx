...
export default function CustomerManagement() {
    ...
    const loadCustomers = async () => {
        try {
            ...
            const result =
                await customerAdminService.getAll(params);
            ...
            <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-[var(--color-text-muted)]">
                        Theo dõi hành vi quay lại và thói quen
                        sử dụng dịch vụ
                    </p>
                </div>
                ...
                <div className="flex flex-col gap-4 border-b border-slate-100 p-5 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-xs text-[var(--color-text-muted)]">
                            Theo dõi số lần ghé và cơ sở khách
                            hàng thường sử dụng
                        </p>
                    </div>
...
function EmptyCustomers() {
    return (
        <div className="py-16 text-center">
            ...
            <p className="mt-1 text-sm text-[var(--color-text-muted)]">
                Thử tìm kiếm bằng tên, số điện thoại hoặc
                email
            </p>
        </div>
    );
...
function CustomerDetail({ customer, onClose }) {
    ...
    useEffect(() => {
        const loadDetail = async () => {
            try {
                ...
                const result =
                    await customerAdminService.getById(
                        customer.id
                    );

                setDetail(result?.data || customer);
            } catch (error) {
                console.error(
    ...
    const branches = Array.isArray(
        ...
        ? [...statistics.branches].sort(
              (a, b) =>
                  (b.visits || 0) -
                  (a.visits || 0)
          )
        : Array.isArray(detail.branches)
        ? [...detail.branches].sort(
              (a, b) =>
                  (b.visits || 0) -
                  (a.visits || 0)
          )
                ...
                {loading ? (
                    ...
                    <>
                        <div className="grid grid-cols-1 gap-4 p-6 sm:grid-cols-3">
                            <DetailStat
                                label="Số lần ghé"
                                value={`${statistics.visitCount || 0} lần`}
                            />
