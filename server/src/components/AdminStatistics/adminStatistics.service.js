...
const getPayments = async (scope, fromDate, toDate) => {
    const { restaurantId, branchId } = scope;
...
const getFoodTrend = async (
    ...
    return {
        increased: increased.slice(0, 5),
        decreased: decreased.slice(0, 5)
    };
};

const getUnsoldFoods = async (scope, currentPayments = []) => {
    console.log("UNSOLD SCOPE:", scope);
...
const getStatistics = async (scope, period, filters = {}) => {
    const { restaurantId, branchId } = scope;
