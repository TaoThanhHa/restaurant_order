...
const create = async (customerId, tableId, items, options = {}) => {
    const { orderId = null, newOrder = false } = options;

    if (!tableId) throw new Error("Thiếu mã bàn.");
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (
            ...
            customer.restaurantId !== table.floor.branch.restaurantId
        ) {
            throw new Error("Khách hàng không thuộc nhà hàng này.");
        }
        ...
        if (orderId) {
            ...
            order = activeOrders.find(
                item => item.id === customer.currentOrderId
            );
        ...
        if (!order && !newOrder && activeOrders.length > 0) {
            const orders = activeOrders.map(currentOrder => ({
                id: currentOrder.id,
                orderCode: currentOrder.orderCode,
                totalAmount: currentOrder.totalAmount,
                createdAt: currentOrder.createdAt,
                customerCount: currentOrder.orderMembers.length,
                itemCount: currentOrder.orderItems.reduce(
                    (total, item) =>
                        total + Number(item.quantity || 0),
                    0
                ),
                members: currentOrder.orderMembers.map(member => ({
                    customerId: member.customerId,
                    customerName:
                        member.customer?.name || "Khách"
                }))
            }));
        ...
        for (const item of items) {
            ...
            await tx.orderItem.create({
                data: {
                    orderId: order.id,
                    foodId: branchFood.food.id,
                    quantity: Number(item.quantity),
                    price: branchFood.food.price,
                    note: item.note || null,
                    status: "PENDING"
                }
            });
