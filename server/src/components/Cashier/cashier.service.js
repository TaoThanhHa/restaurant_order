...
const dashboard = async (branchId) => {
    ...
    const [openOrders, availableTables, occupiedTables, payments] = await Promise.all([
        prisma.order.count({
            where: {
                branchId,
    ...
    const todayRevenue = payments.reduce((sum, item) => sum + Number(item.totalAmount), 0);

    return { todayRevenue, openOrders, availableTables, occupiedTables };
};
...
const notifyOrderCustomers = async (orderId, event = "order.updated") => {
    ...
    const customerIds = [...new Set(
        order.orderMembers.map(item => item.customerId).filter(Boolean)
    )];
...
const getPendingOrders = async (branchId) => {
    ...
    return await prisma.order.findMany({
        where: {
            branchId,
            status: "PENDING",
            orderItems: { some: { status: "PENDING" } }
        },
...
const getServingOrders = async (branchId) => {
    ...
    const servingStatuses = ["CONFIRMED", "PREPARING", "READY", "SERVED"];

    return await prisma.order.findMany({
        where: {
            branchId,
            status: "CONFIRMED",
            orderItems: { some: { status: { in: servingStatuses } } }
        },
        ...
        include: {
            ...
            orderItems: {
                where: { status: { in: servingStatuses } },
                include: { food: true },
                orderBy: { id: "asc" }
            }
        }
...
const updateOrderItemStatus = async (branchId, itemId, status) => {
    ...
    if (next !== status) {
        throw new Error(`Không thể chuyển từ ${item.status} sang ${status}.`);
    }
...
const payment = async (orderId, data) => {
    ...
    const items = order.orderItems.filter(item => item.status !== "CANCELLED");

    if (items.length === 0) 
        throw new Error("Hóa đơn chưa có món.");

    const unfinished = items.find(item => item.status !== "SERVED");

    if (unfinished) {
        throw new Error("Vẫn còn món chưa phục vụ, không thể thanh toán.");
    }
...
const getStatistics = async (branchId) => {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const dayPayments = payments.filter(payment => {
            ...
            return getLocalDateString(new Date(payment.paidAt)) === dateString;
        });

        const memberRevenue = dayPayments
            .filter(payment => {
                const members = payment.order?.orderMembers || [];
                return members.some(
                    member => member.customer && member.customer.isGuest === false
                );
            })
            .reduce((sum, payment) => sum + Number(payment.totalAmount || 0), 0);

        const guestRevenue = dayPayments
            .filter(payment => {
                const members = payment.order?.orderMembers || [];
                return !members.some(
                    member => member.customer && member.customer.isGuest === false
        ...
        const dineInPayments = dayPayments.filter(payment => {
            const orderType = String(payment.order?.orderType || "")
                .trim()
                .toUpperCase()
                .replace(/[-\s]/g, "_");

            return orderType === "DINE_IN" || orderType === "DINEIN";
        });

        const takeAwayPayments = dayPayments.filter(payment => {
            const orderType = String(payment.order?.orderType || "")
                .trim()
                .toUpperCase()
                .replace(/[-\s]/g, "_");
        ...
        const dineInRevenue = dineInPayments.reduce(
            (sum, payment) => sum + Number(payment.totalAmount || 0),
            0
        );

        const takeAwayRevenue = takeAwayPayments.reduce(
            (sum, payment) => sum + Number(payment.totalAmount || 0),
            0
        );

        const total = dineInRevenue + takeAwayRevenue;

        console.log(`[STATISTICS] ${dateString}`, {
            total,
