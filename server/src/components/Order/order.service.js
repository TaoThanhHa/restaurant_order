...
const mergeOrders = async ({ targetOrderId, sourceOrderIds, user }) => {
    targetOrderId = Number(targetOrderId);
    sourceOrderIds = sourceOrderIds.map(Number).filter(id => id !== targetOrderId);

    if (!targetOrderId) throw new Error("Vui lòng chọn đơn chính");
    if (!sourceOrderIds.length) throw new Error("Vui lòng chọn ít nhất một đơn để gộp");

    return prisma.$transaction(async tx => {
        const orders = await tx.order.findMany({
            where: { id: { in: [targetOrderId, ...sourceOrderIds] } },
            include: { orderItems: true, orderMembers: true },
        });
        ...
        if (!targetOrder) throw new Error("Không tìm thấy đơn chính");
        ...
        if (!orders.every(order => order.sessionId === targetOrder.sessionId)) {
            throw new Error("Chỉ có thể gộp các đơn thuộc cùng một bàn");
        }
        ...
        for (const sourceOrderId of sourceOrderIds) {
            ...
            await tx.payment.deleteMany({ where: { orderId: sourceOrderId } });
        ...
        await notifyOrderCustomers(targetOrderId);
        return result;
    });
};
